...
dr_obs = [-1,0]
min_oa, max_oa = 15,35
min_temp, max_temp = 273.15+18, 273.15+33
...
for episode in range(last_ep+1,last_ep+episodes+1):
    ...
    if test==True: 
        day_no = np.random.choice(test_days)
    else: 
    ...
    start_time = 24 * 3600 * day_no #+ 3600*23
    count = 0    
    ...
    agent_state_n = np.array([mean_T_n, state_n[5],state_n[6],state_n[7],state_n[8]])
    ...
    for i in range(int(episode_length/step)):           
        ...
        oa_air = forecasts['TDryBul'][0]       
        mean_T = sum([building_states[i] for i in building_obs[:-1]])/5      

        print ("dr_m1: {}".format(dr_m1))
        ...
        '''Agent Related'''      
        act = rbc_controller(hours=hours,mean_T=mean_T,DR_time=np.array(DR_time))
        ...
        state = [(float(x)*(upper_obs_bounds[y]-lower_obs_bounds[y])+lower_obs_bounds[y]) for x,y in zip(state_n,index)] # General State                
        ...
        print(' Day no: {}, Hours: {}, i {}, Count: {}'.format(day_no, hours, i, count))
        print("DR Start and End : {}".format([dr_rand_start,dr_rand_end]))        
        print ("State : {}".format(state)) ;print () 
        ...
        next_agent_state_n = np.array([next_mean_T_n, next_state_n[5],next_state_n[6],next_state_n[7],next_state_n[8]])  
        ...
        print ("next_dr_m1: {}, hours: {}".format(next_dr_m1,hours))
        ...
        next_agent_state = [(float(x)*(upper_obs_bounds[y]-lower_obs_bounds[y])+lower_obs_bounds[y]) for x,y in zip(next_agent_state_n,index)] # General State 
        ...
        reward =  w[0]*energy_cost + w[1]*tdisc_cost+ w[2]*ppen_cost
        ...
        print(next_agent_state,reward, done, info)
        print()
        ...
        print("Room Temperature;  Zone 0: {}, Zone 1: {}, Zone 2: {}, Zone 3: {}, Zone 4: {}, Mean Temp: {}".format(
            ...
            building_states['senTemRoom4_y'],mean_temp_n))
        print ()
        ...
        ''' Extra info storage'''                 
        extra_info['s0'].append(next_index_states[0])
        extra_info['s1'].append(next_index_states[1])
        extra_info['s2'].append(next_index_states[2])
        extra_info['s3'].append(next_index_states[3])
        ...
        if done:
            ...
            KPI_df.to_csv(path+"01_KPI/KPI_" + str(episode) + "_rbc.csv")
