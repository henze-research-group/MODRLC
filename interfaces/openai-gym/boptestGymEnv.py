...
class BoptestGymEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.no_of_states  =  len(self.building_obs) + len(self.dr_obs) + len(list(itertools.chain(*list(forecast_obs.values()))))

        self.state_list = []
                
        for i in range(1,self.no_of_states+1):
            self.state_list.append('s'+str(i))

        if self.n_obs==True:
            self.state_n_list = []
            for i in range (1,self.no_of_states+1):
                self.state_n_list.append('s'+str(i)+'_n')                   
                    
        self.historian = pd.DataFrame(columns=(self.historian_keys.tolist()+self.state_list+self.state_n_list))
        ...
        # print ("Lower Act Bounds")
        # print (self.lower_act_bounds)

        # print ("Upper Act Bounds")
        # print (self.upper_act_bounds)
    ...
    def store_data(self):
        ...
        states,states_n = self.get_states(self.building_y_previous ,self.forecast_y_previous)

        self.historian.loc[len(self.historian['state'])-self.historian['state'].isna().sum(),'state'] = states
        ...
        for state_no in range(len(self.state_list)):
            # print ('State no: {}'.format(state_no))
            # print ("state key check: {}".format(self.state_list[state_no]))
            self.historian.loc[len(self.historian[self.state_list[state_no]]) - self.historian[self.state_list[state_no]].isna().sum(),self.state_list[state_no]] = states[state_no]
            if self.n_obs == True:
                self.historian.loc[len(self.historian[self.state_list[state_no]+'_n']) - self.historian[self.state_list[state_no]+'_n'].isna().sum(),self.state_list[state_no]+'_n'] = states_n[state_no]     
    ...
    def reset(self):
        ...
        states,states_n = self.get_states(self.building_y ,self.forecast_y)
    ...
    def step(self, action):
        ...
        # print ("Check 01")
        # print (self.testcase_info[0])

        u_cool_setp = self.testcase_info[0]["u_cool_stp"]
        u_htg_setp = self.testcase_info[0]["u_htg_stp"]

        for cool_stp in u_cool_setp:
            if cool_stp in (self.u).keys():
                if self.u[cool_stp]<= self.lower_sp:
                    htg_stp = cool_stp.replace('Coo','Hea')
                    self.u[htg_stp] = self.lower_sp -3
                    self.u[htg_stp.replace('_u','_activate')] = 1 

        for htg_stp in u_htg_setp:
            if htg_stp in (self.u).keys():
                if self.u[htg_stp]>= self.upper_sp:
                    cool_stp = htg_stp.replace('Hea','Coo')
                    self.u[cool_stp] = self.upper_sp +3
                    self.u[cool_stp.replace('_u','_activate')] = 1 
    ...
    def save_episode(self,filename,extra_info=False):
        self.extra_info = extra_info       

        for k in self.extra_info.keys():
            print ("{}: {}".format(k,len(self.extra_info[k])))

        if self.extra_info==False:
            pass
        else:             
            self.historian = pd.concat([self.historian,pd.DataFrame.from_dict(self.extra_info)],axis=1)        
    ...
    def get_DR_signal(self):
        ...
        if countdown_dr>self.dr_signal_time:
            countdown_dr =0 
        elif countdown_dr>0 and countdown_dr<=self.dr_signal_time:
            countdown_dr = self.dr_signal_time - (countdown_dr)
        else:
            countdown_dr = 0

                      
        countdown_dr_n = countdown_dr/self.dr_signal_time        
        DR_signal_n = DR_signal.copy()

        if self.dr_pos_countdown!=None:
            DR_signal.insert(self.dr_pos_countdown, countdown_dr)
            DR_signal_n.insert(self.dr_pos_countdown, countdown_dr_n) 
            self.dr_obs.insert(self.dr_pos_countdown,-1)


        return DR_signal,DR_signal_n
    ...
    def get_states(self, building_y, forecasts):
        ...
        observations_n = self.normalize_obs(observations)

        if self.DR_event == True:
            DR_signal,DR_signal_n = self.get_DR_signal()
            for obs in DR_signal:                
                observations.append(obs)
            for obs_n in DR_signal_n:
                observations_n.append(obs_n)


        # Reformat observations
        states = np.array(observations).astype(np.float32)
        states_n = np.array(observations_n).astype(np.float32)

        print ()

        return states,states_n
    ...
    def plot_episode(self,plot_name,zones):
        ...
        low_rect, upp_rect, low_rect_p, upp_rect_p = -30,50,0,40000
        ...
        for zone in zones:
            ...
            rect_temp_list.append(Polygon(((self.DR_time[0]/3600, low_rect), (self.DR_time[0]/3600, upp_rect), (self.DR_time[1]/3600, upp_rect), (self.DR_time[1]/3600, low_rect)),
                         fc=(1, 0, 0, 0.1), ec=(0, 0, 0, 1), lw=0.25, linestyle='--'))
        ...
        if self.DR_event == True:
            rect_temp_list.append(Polygon(((self.DR_time[0]/ 3600, low_rect), (self.DR_time[0]/3600, upp_rect),
                                          ...
                                          fc=(1, 0, 0, 0.1), ec=(0, 0, 0, 1), lw=0.25, linestyle='--'))
            rect_power_list.append(Polygon(((self.DR_time[0] / 3600, low_rect_p), (self.DR_time[0] / 3600, upp_rect_p),
                                            (self.DR_time[1] / 3600, upp_rect_p), (self.DR_time[1] / 3600, low_rect_p)),
                                           fc=(1, 0, 0, 0.1), ec=(0, 0, 0, 1), lw=0.25, linestyle='--'))
        ...
        if plot_total==True:
            ...
            x2a.grid(which='both', linewidth=0.5, color='white',zorder=3)
            x2a.grid(which='major', linewidth=2, color='white',zorder=3)
            ...
            x2a.plot(self.historian['senHouDec_y'],
                     ...
                     label='Mean Temp of all Zones[C]', color="steelblue", linewidth=2,zorder=11)
            ...
            x2b.axvline(x=self.DR_time[0], color='k', linestyle='--', linewidth=1, dashes=(2, 2),zorder=7)
            x2b.axvline(x=self.DR_time[1], color='k', linestyle='--', label='DR Event Interval', linewidth=1, dashes=(2, 2),zorder=7)
            ...
            x2b.plot(self.historian['senHouDec_y'],
                     self.historian["senPowTot_y"], color='r',label="Total HVAC Power")
            x2b.legend(loc='upper right')
